...
@section('content')
<!-- LINK FONT & TAILWIND CSS -->
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800;900&display=swap" rel="stylesheet">
<script src="https://cdn.tailwindcss.com"></script>

<style>
    /* PENGATURAN FONT: Mengunci font Inter agar tampil profesional */
    * { font-family: 'Inter', sans-serif; }
    body { font-family: 'Inter', sans-serif; }

    /* SCROLLBAR: Mempercantik tampilan scroll di daftar progres */
    .progress-list-scroll::-webkit-scrollbar { width: 5px !important; height: 5px !important; }
    .progress-list-scroll::-webkit-scrollbar-track { background: transparent; }
    .progress-list-scroll::-webkit-scrollbar-thumb { background: #cbd5e1 !important; border-radius: 10px !important; }

    .figma-shadow-card { box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.01), 0 2px 4px -1px rgba(0, 0, 0, 0.01); }
    .live-row-card { transition: all 0.2s ease; }

    /* ANIMASI KOTAK STATISTIK: Efek slow motion saat kursor masuk */
    .slow-motion-box { transition: all 0.4s cubic-bezier(0.25, 1, 0.5, 1) !important; }
    .slow-motion-box:hover { transform: translateY(-4px) scale(1.01); box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.04), 0 10px 10px -5px rgba(0, 0, 0, 0.02); }

    /* WARNA STATUS: Indikator warna untuk setiap tahap progres */
    ...
    /* ANIMASI DROPDOWN: Efek muncul yang lembut */
    @keyframes popIn { from { opacity: 0; transform: scale(0.95) translateY(5px); } to { opacity: 1; transform: scale(1) translateY(0); } }
    .animate-dropdown { animation: popIn 0.15s cubic-bezier(0.16, 1, 0.3, 1) forwards; }
</style>
    ...
    <div class="w-full flex-1 flex flex-col p-4 gap-3.5 overflow-hidden">
        ...
        <div class="w-full flex flex-col sm:flex-row gap-3 justify-between items-center flex-shrink-0">
            <div class="flex items-center bg-white p-1 rounded-xl border border-slate-200 shadow-sm gap-1">
                <button type="button" onclick="setKategoriTab('ALL', this)" class="tab-btn bg-blue-600 text-white font-extrabold text-[9px] px-4 py-2 rounded-lg uppercase tracking-wide transition-all shadow-sm">Semua Unit</button>
                <button type="button" onclick="setKategoriTab('PROSES', this)" class="tab-btn text-slate-500 hover:text-slate-900 font-bold text-[9px] px-4 py-2 rounded-lg uppercase tracking-wide transition-all">Sedang Dicuci</button>
                <button type="button" onclick="setKategoriTab('READY', this)" class="tab-btn text-slate-500 hover:text-slate-900 font-bold text-[9px] px-4 py-2 rounded-lg uppercase tracking-wide transition-all">Selesai</button>
            </div>

            <div class="w-full sm:w-64 relative">
                <input type="text" id="search-plat-nama" onkeyup="jalankanSistemFilterProgresKombinasi()" placeholder="Cari Plat Nomor / Nama..." class="w-full bg-white border border-slate-200 rounded-xl pl-3 pr-8 py-2 text-xs font-bold text-slate-800 placeholder-slate-400 focus:outline-none focus:border-blue-600 transition-all shadow-sm">
                <span class="absolute right-3 top-2.5 text-slate-400 text-xs">🔍</span>
            </div>
        </div>

        <!-- DAFTAR LIST -->
        <div id="progress-list-wrapper" class="w-full flex-1 flex flex-col gap-2.5 overflow-y-auto progress-list-scroll pr-1"></div>

        <!-- FOOTER STATISTIK -->
        <div class="grid grid-cols-4 gap-4 flex-shrink-0">
            <!-- Statistik 1: Estimasi Waktu -->
            ...
            <!-- Statistik 3: Proses Pengeringan -->
...
<script>
    ...
    /* SINKRONISASI DATA DARI CONTROLLER */
    let arrayDatabaseProgresWorkshop = @json($antreanAktif ?? []);

    /* FUNGSI MAPPING: Mengatur status unit */
    arrayDatabaseProgresWorkshop = arrayDatabaseProgresWorkshop.map(item => {
        let stepAwal = 1; let persenAwal = 5; let labelAwal = "ANTREAN"; let badgeColorAwal = "bg-slate-100 text-slate-700";
        ...
        if (item.status === 'PENCUCIAN' || item.status === 'CUCI') {
            ...
            stepAwal = 1; persenAwal = 5; labelAwal = "ANTREAN"; badgeColorAwal = "bg-slate-100 text-slate-700";
        }

        return { id: item.id, tgl: "SINKRON", nopol: item.plat_nomor, nama: item.nama_pelanggan, mobil: item.jenis_kendaraan ?? 'MOBIL', paket: item.paket_cuci ?? 'REGULER', masuk: new Date(item.created_at).toLocaleTimeString('id-ID', {hour: '2-digit', minute:'2-digit'}) + " WIB", step: stepAwal, persen: persenAwal, badgeColor: badgeColorAwal, labelBadge: labelAwal };
    });

    /* FUNGSI TAB: Switch tampilan */
    function setKategoriTab(kategori, element) {
        currentTabKategori = kategori;
        document.querySelectorAll('.tab-btn').forEach(btn => { btn.className = "tab-btn text-slate-500 hover:text-slate-900 font-bold text-[9px] px-4 py-2 rounded-lg uppercase tracking-wide transition-all"; });
    ...
    function kalkulasiFooterStatistik(dataTerfilter) {
        let totalAntrean = 0; let totalCuci = 0; let totalKering = 0;
    ...
    /* FUNGSI UPDATE STATUS: Mengirim update ke Database */
    function eksekusiGantiProgresManual(idMobil, stepBaru) {
        ...
        if (index === -1) return;
        let statusText = (stepBaru === 1) ? 'ANTREAN' : (stepBaru === 2) ? 'PENCUCIAN' : (stepBaru === 3) ? 'PENGERINGAN' : 'READY';
    ...
    /* FUNGSI TOGGLE DROPDOWN: Menu pilihan status unit */
    ...
    /* FUNGSI RENDER BOARD: Menampilkan data ke layar */
    function renderLiveProgressBoard(dataToRender) {
        const wrapper = document.getElementById('progress-list-wrapper');
        wrapper.innerHTML = "";
        if (dataToRender.length === 0) {
            wrapper.innerHTML = `<div class="flex flex-col items-center justify-center p-12 text-center bg-white border border-slate-200 rounded-2xl flex-1 text-slate-400 font-medium italic shadow-sm"><span class="text-2xl mb-1">📋</span><span class="text-[10px]">Belum ada unit pengerjaan aktif dari kasir pada periode ini, Chief.</span></div>`;
            kalkulasiFooterStatistik([]); return;
        }
        dataToRender.forEach(row => {
            ...
            const line1 = row.step >= 2 ? 'bg-amber-500' : 'bg-slate-200'; const line2 = row.step >= 3 ? 'bg-blue-600' : 'bg-slate-200'; const line3 = row.step >= 4 ? 'bg-emerald-500' : 'bg-slate-200';
            let colorPersenClass = row.step === 1 ? "text-slate-400" : (row.step === 2 ? "text-amber-500" : (row.step === 3 ? "text-blue-600" : "text-emerald-500"));
            ...
            wrapper.innerHTML += `
                <div class="live-row-card bg-white border border-slate-200 rounded-2xl px-4 py-3 shadow-sm flex items-center justify-between gap-1 flex-shrink-0 relative">
                    <div class="flex items-center gap-3 w-[30%] flex-shrink-0">
                        <div class="p-2.5 bg-blue-600 border border-blue-700 rounded-xl text-white flex-shrink-0 shadow-[0_4px_10px_rgba(0,0,0,0.15)]"><svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M19 17h2c.6 0 1-.4 1-1v-3c0-.9-.7-1.7-1.5-1.9C18.7 10.6 16 10 16 10s-1.3-1.4-2.2-2.3c-.5-.4-1.1-.7-1.8-.7H5c-.6 0-1.1.4-1.4.9l-1.4 2.9A3.7 3.7 0 0 0 2 12v4c0 .6.4 1 1 1h2" /><circle cx="7" cy="17" r="2" /><circle cx="17" cy="17" r="2" /></svg></div>
                        <div class="flex flex-col gap-0.5 truncate"><div class="flex items-center gap-1.5 flex-wrap"><span class="text-xs font-extrabold text-slate-700 tracking-tight">${row.nama}</span><span class="${row.badgeColor} text-[6.5px] font-black px-1 py-0.5 rounded uppercase tracking-wide shadow-sm">${row.labelBadge}</span></div><div class="text-[9px] font-extrabold text-slate-500 uppercase tracking-wide mt-0.5">${row.mobil} <span class="text-slate-300 font-bold">|</span> <span class="text-blue-600 font-black">${row.nopol}</span></div><div class="text-[8px] font-bold text-slate-400 uppercase tracking-wider mt-0.5"><span class="bg-slate-50 border border-slate-100 px-1 py-0.5 rounded text-slate-500 font-extrabold">${row.paket}</span> <span>⏱️ ${row.masuk}</span></div></div>
                    </div>
                    <div class="w-[58%] flex items-center justify-center px-1 flex-shrink-0">
                        <div class="w-full flex items-center relative">
                            <div class="flex flex-col items-center z-10 relative"><div class="h-7 w-7 rounded-full border-2 ${s1Class} flex items-center justify-center shadow-sm">${row.step >= 1 && row.step !== 1 ? '✓' : '1'}</div><span class="text-[7px] font-black text-slate-400 uppercase tracking-wider mt-1.5 absolute top-6">Antrean</span></div><div class="flex-1 h-[3px] ${line1} mx-1"></div>
                            <div class="flex flex-col items-center z-10 relative"><div class="h-7 w-7 rounded-full border-2 ${s2Class} flex items-center justify-center shadow-sm">${row.step >= 2 && row.step !== 2 ? '✓' : '2'}</div><span class="text-[7px] font-black text-slate-400 uppercase tracking-wider mt-1.5 absolute top-6">Cuci</span></div><div class="flex-1 h-[3px] ${line2} mx-1"></div>
                            <div class="flex flex-col items-center z-10 relative"><div class="h-7 w-7 rounded-full border-2 ${s3Class} flex items-center justify-center shadow-sm">${row.step >= 3 && row.step !== 3 ? '✓' : '3'}</div><span class="text-[7px] font-black text-slate-400 uppercase tracking-wider mt-1.5 absolute top-6">Kering</span></div><div class="flex-1 h-[3px] ${line3} mx-1"></div>
                            <div class="flex flex-col items-center z-10 relative"><div class="h-7 w-7 rounded-full border-2 ${s4Class} flex items-center justify-center shadow-sm">${row.step >= 4 && row.step !== 4 ? '✓' : '4'}</div><span class="text-[7px] font-black text-slate-400 uppercase tracking-wider mt-1.5 absolute top-6">Finish</span></div>
                        </div>
                    </div>
                    <div class="w-[12%] flex items-center justify-start pl-4 gap-3 relative flex-shrink-0"><div class="text-left"><span class="block text-xl font-black tracking-tighter leading-none ${colorPersenClass}">${row.persen}%</span><span class="text-[7px] font-bold text-slate-400 uppercase tracking-wider mt-0.5 block">SELESAI</span></div><div class="relative"><button type="button" onclick="toggleDropdownMenuBox('dropdown-box-id-${row.id}')" class="text-slate-400 hover:text-slate-800 transition-all font-black p-1.5 text-base rounded-lg hover:bg-slate-100">⋮</button><div id="dropdown-box-id-${row.id}" class="dropdown-menu-box hidden absolute right-0 mt-1 w-40 bg-white border border-slate-200 rounded-xl shadow-xl z-50 py-1.5 animate-dropdown"><div class="px-2.5 py-1 text-[7px] font-black text-slate-400 uppercase tracking-wider border-b border-slate-100">Ubah Progres Kerja</div><button type="button" onclick="eksekusiGantiProgresManual(${row.id}, 1)" class="w-full text-left px-3 py-1.5 text-[9px] font-bold text-slate-600 hover:bg-slate-50 flex items-center gap-1.5">⭕ Antrean</button><button type="button" onclick="eksekusiGantiProgresManual(${row.id}, 2)" class="w-full text-left px-3 py-1.5 text-[9px] font-bold text-slate-600 hover:bg-slate-50 flex items-center gap-1.5 text-amber-500">⏳ Masuk Ruang Cuci</button><button type="button" onclick="eksekusiGantiProgresManual(${row.id}, 3)" class="w-full text-left px-3 py-1.5 text-[9px] font-bold text-slate-600 hover:bg-slate-50 flex items-center gap-1.5 text-blue-600">💨 Mulai Keringkan</button><button type="button" onclick="eksekusiGantiProgresManual(${row.id}, 4)" class="w-full text-left px-3 py-1.5 text-[9px] font-bold text-slate-600 hover:bg-emerald-50 flex items-center gap-1.5 text-emerald-500">✨ Selesai (Ready)</button></div></div></div>
                </div>`;
        });
        kalkulasiFooterStatistik(dataToRender);
    }

    /* FUNGSI FILTER: Pencarian unit */
    ...
    window.addEventListener('click', function(e) { if (!e.target.matches('button')) document.querySelectorAll('.dropdown-menu-box').forEach(el => el.classList.add('hidden')); });
